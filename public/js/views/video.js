window.VideoView = Backbone.View.extend({
	tagName : "div",
	className : "videoContainer",
	
    initialize : function() {
		this.model.bind("reset", this.render, this);
		this.model.bind("add", this.render, this);
	},
	
	render : function(eventName) {
		_.each(this.model.models, function(content) {
			if(this.options.activityMode == 2) {
				$(this.el).append(new VideoItemView({model : content, activityMode:this.options.activityMode}).render().el);
			}
			else if(this.options.activityMode == 1) {
				$('#activityContainer').append(new VideoItemView({model : content, activityMode:this.options.activityMode}).render().el);
			}
		}, this);
		return this;
	}
});

window.VideoItemView = Backbone.View.extend({
	
	initialize : function() {
		_.bindAll(this, "createVideoComments");
		this.template = _.template(tpl.get('video_tpl'));
		this.model.bind("change", this.render, this);
	},
	
	updatePosition : function(event) {
		var pleft = this.el.style.left;
		var ptop = this.el.style.top;
		this.model.attributes.wxpos = pleft.substring(0, pleft.length-2);;
		this.model.attributes.wypos = ptop.substring(0, ptop.length-2);
		this.model.save();
	},
	
	
	events : {
		"dragstop" : "updatePosition",
		"keypress .commentInput" : "handleKeyPress",
		"click .commentInput" : "handleOnfocus",
		"focusout .commentInput" : "handleOnfocusout"
	},
	
	handleKeyPress:function(event) {
		var key = event.keyCode;
		if(key==13 && event.currentTarget.value!='') {
			var vc = new VideoComment();
			vc.attributes.fbcontent = event.currentTarget.value;
			vc.attributes.vid_id = this.model.id;
			vc.save({wait: true});
			this.commentModel.add(vc);
		}
	},
	
	handleOnfocus:function(event) {
		event.currentTarget.value = '';
	},
	
	handleOnfocusout:function(event) {
		event.currentTarget.value = 'Write a comment...';
	},
	
	createVideoComments: function(m, response) {
		this.commentModel = m;
		this.commentModel.bind("add", this.refreshVideoComments, this);
		_.each(this.commentModel.models, function(comment) {
			$('#videoComment-'+this.model.id).append(new CommentItemView({ model : comment }).render().el);
		}, this);
	},
	
	refreshVideoComments : function() {
		$('#videoComment-'+this.model.id).html('');
		this.videoCommentList = new VideoCommentCollection();
		this.videoCommentList.fetch({ data: $.param({ vid_id: this.model.id }),
			success : this.createVideoComments,
			wait: true
		});
		$('.commentInput').blur();
	},

	render : function(eventName) {
		$(this.el).html(this.template(this.model.toJSON()));
		
		this.refreshVideoComments();
		
		if(this.options.activityMode == 1) {
			$(this.el).attr('style', 'left:' + this.model.attributes.wxpos + 'px;top:' + this.model.attributes.wypos + 'px');
			$(this.el).addClass('video-ui');
			$(this.el).draggable({
				//handle : '.toolbar',
				stack: "activityContainer",
				containment: 'activityContainer'
			});
		}
		else if(this.options.activityMode == 2) {
			$(this.el).addClass('video-ui-tl');
		}
		return this;
	}
});